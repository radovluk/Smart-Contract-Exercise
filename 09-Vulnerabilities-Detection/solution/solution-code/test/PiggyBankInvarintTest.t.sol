...
contract PiggyBankInvariantTest is Test {
    ...
    function invariant_balanceMatchesAccountingDiff() public view {
        // Skip check if no operations were performed
        if (!handler.operationsPerformed()) return;
    ...
    /**
     * @notice Invariant #2: Handler's tracked deposits should match contract's totalDeposits
    ...
    /**
     * @notice Invariant #3: Handler's tracked withdrawals should match contract's totalWithdrawals
     * @dev This ensures the internal state tracking is accurate
     */
    ...
    /**
     * @notice Invariant #4: Total withdrawals should never exceed total deposits
    ...
    function invariant_callSummary() public view {
        console2.log("===== PiggyBank Invariant Test Statistics =====");
        console2.log("Total deposits:", handler.totalDeposited());
        console2.log("Total withdrawals:", handler.totalWithdrawn());
        console2.log("Contract balance:", address(piggyBank).balance);
    }
}
...
contract PiggyBankHandler is Test {
    ...
    // Flag to ensure we've run at least one operation
    bool public operationsPerformed;
    ...
    function deposit(uint256 actorIdx, uint256 amount) external {
        ...
        // Bound amount to a reasonable range (0.001 ETH to 5 ETH)
        amount = bound(amount, 0.001 ether, 5 ether);
        
        // Ensure actor has enough ETH
        if (actor.balance < amount) {
            amount = actor.balance / 2; // Use half their balance if not enough
        }
        ...
        // Flag that we've performed at least one operation
    ...
    function withdraw(uint256 amount) external {
        // Only the owner can withdraw, skip if the current contract balance is 0
        if (address(piggyBank).balance == 0) return;
        
        // Bound amount to something within the current balance
        amount = bound(amount, 0.001 ether, address(piggyBank).balance);
    ...
    function withdrawAsNonOwner(uint256 actorIdx, uint256 amount) external {
        ...
        // Skip if the current contract balance is 0
        if (address(piggyBank).balance == 0) return;
