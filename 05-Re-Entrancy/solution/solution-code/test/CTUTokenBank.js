...
describe("CTUTokenBank Challenge", function () {
    ...
    before("Set up the challenge", async function () {
        ...
        token = await (await ethers.getContractFactory("CTUToken", deployer)).deploy();

        // Deploy the CTUTokenBank contract, passing it the CTUToken address
        bank = await (await ethers.getContractFactory("CTUTokenBank", deployer)).deploy(token.target);

        // Transfer all tokens from deployer to the bank
        await token.connect(deployer).transfer(bank.target, CTU_TOKEN_INITIAL_SUPPLY);
        expect(await token.balanceOf(bank.target)).to.equal(CTU_TOKEN_INITIAL_SUPPLY);

        // Set the bank’s initial ETH balance
        await setBalance(bank.target, BANK_INITIAL_ETH_BALANCE);
        expect(await ethers.provider.getBalance(bank.target)).to.equal(BANK_INITIAL_ETH_BALANCE);
    ...
    it("Execution of the player's code", async function () {
        ...
        const attackerContractFactory = await ethers.getContractFactory("CTUTokenBankAttacker", player);
        const attackerContract = await attackerContractFactory.deploy(bank.target, token.target);
    ...
    after("Success conditions", async function () {
        ...
        expect(playerBalance).to.be.gt(15, "Player did not gain funds from the bank");
    });
});
