...
describe("CatCharity Challenge", function () {
    ...
    before("Set up the challenge", async function () {
        ...
        catCharity = await (await ethers.getContractFactory("CatCharity", deployer)).deploy();
        await catCharity.donate({ value: CHARITY_INITIAL_BALANCE });

        // Verify that charity starts with 10 ETH
        expect(await ethers.provider.getBalance(catCharity.target)).to.equal(CHARITY_INITIAL_BALANCE);
    ...
    it("Execution of the player's code", async function () {
        ...
        catAttacker = await (await ethers.getContractFactory("CatAttacker", deployer)).deploy(catCharity.target);
