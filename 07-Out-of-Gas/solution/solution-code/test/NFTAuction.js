...
describe("NFT Auction Challenge", function () {
  ...
  before("Set up the challenge", async function () {
    ...
    [deployer, seller, player, bidder1, bidder2, bidder3] =
    ...
    const mintTx = await felStudentNFT
      .connect(deployer)
      .mint(seller.address, programIndex, sleepIndex, toolIndex);
    ...
    await felStudentNFT
      .connect(seller)
      .transferFrom(seller.address, nftAuction.target, tokenId);
  ...
  it("Execution of the player's code", async function () {
    ...
    const NFTAuctionAttacker = await ethers.getContractFactory(
      "NFTAuctionAttacker",
      player
    );
  ...
  after("Success conditions", async function () {
    ...
    try {
      ...
      console.log(
        "All bidder funds and the NFT are now stuck in the contract forever!"
      );
    ...
    expect(
      await felStudentNFT.ownerOf(tokenId),
      "The bidder 3 should not own the NFT!"
    ).to.not.equal(bidder3.address);

    // Verify the NFT is still owned by the auction contract
    expect(
      await felStudentNFT.ownerOf(tokenId),
      "NFT should remain with the auction contract"
    ).to.equal(nftAuction.target);

    // Verify the auction is still active
    expect(
      await nftAuction.ended(),
    ...
    ).to.be.lessThanOrEqual(BIDDER1_INITIAL_ETH_BALANCE - bidder1TotalBid);
    expect(
      bidder2Balance,
      "Bidder2 balance should be reduced by the sum of his bids"
    ).to.be.lessThanOrEqual(BIDDER2_INITIAL_ETH_BALANCE - bidder2TotalBid);
    ...
    ).to.be.greaterThan(bidder1TotalBid + bidder2TotalBid);
  });
});
