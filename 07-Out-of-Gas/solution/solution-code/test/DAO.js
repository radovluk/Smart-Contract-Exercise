...
describe("DAO Challenge", function () {
  ...
  before("Set up the challenge", async function () {
    ...
    [deployer, player, member1, member2, member3, member4] =
    ...
    console.log(
      `Cat Charity proposal has ${proposal[5]} votes out of ${memberCount} members`
    );
    expect(proposal[5]).to.be.above(memberCount / 2n);

    console.log(
      "Initial DAO Treasury Balance:",
      ethers.formatEther(await dao.treasuryBalance()),
      "ETH"
    );
    console.log(
      "Cat Charity Initial Balance:",
      ethers.formatEther(await ethers.provider.getBalance(catCharity.target)),
      "ETH"
    );
  });
  ...
  it("Execution of the player's code", async function () {
    ...
    console.log(
      `Adding ${NUM_PROPOSALS} proposals in batches of ${BATCH_SIZE}...`
    );

    for (
      let batchStart = 0;
      batchStart < NUM_PROPOSALS;
  ...
  after("Success conditions", async function () {
    ...
    expect(
      catCharityProposal[6],
      "The Cat Charity proposal should not have been executed"
    ).to.be.false;

    // Verify the treasury funds are still safe in the DAO
    const finalDAOBalance = await dao.treasuryBalance();
    console.log(
      "Final DAO Treasury Balance:",
      ethers.formatEther(finalDAOBalance),
      "ETH"
    );
    expect(
      finalDAOBalance,
      "Treasury balance should not have decreased"
    ).to.be.at.least(INITIAL_TREASURY);

    // Verify the Cat Charity didn't receive any funds
    const catCharityBalance = await ethers.provider.getBalance(
      catCharity.target
    );
    console.log(
      "Final Cat Charity Balance:",
      ethers.formatEther(catCharityBalance),
      "ETH"
    );
    expect(
      catCharityBalance,
      "Cat Charity should not have received any funds"
    ).to.equal(0);
