...
describe("NFT Auction Frontrunning Challenge", function () {
  ...
  it("Execution of the player's code", async function () {
     ...
     *
     * Fee = Gas Used × (Base Fee + min(maxPriorityFeePerGas, maxFeePerGas - baseFee))
     *
     * Where:
     * - Base Fee: Algorithmically determined by the network based on demand
     *   - This amount is BURNED (removed from circulation)
     *   - Increases/decreases by up to 12.5% per block depending on block fullness
     *
     * - Priority Fee: Set by the user as maxPriorityFeePerGas
     *   - This amount goes to miners/validators as an incentive
     *   - Higher priority fees get transactions included faster
     *
     * User-specified parameters:
     * - maxFeePerGas: Maximum total fee user is willing to pay per gas unit
     * - maxPriorityFeePerGas: Maximum priority fee user is willing to pay per gas unit
     *
     * - If baseFee + maxPriorityFeePerGas > maxFeePerGas, priority fee is reduced
     * - If baseFee > maxFeePerGas, transaction becomes invalid and is rejected
     *
     * For frontrunning: Setting a higher maxPriorityFeePerGas than the target
     * transaction ensures miners prioritize including your transaction first
     */

    // Prepare frontrunning transaction with EIP-1559 format
    ...
    const frontrunTx = {
      ...
      gasLimit: 500000, // Ensure enough gas is provided, need to be set
    };
    ...
    // Check the mempool to see both transactions
    pendingBlock = await network.provider.send("eth_getBlockByNumber", [
      "pending",
      false,
    ]);
    ...
    // Use our explorer function to show detailed transaction info
    await exploreMempoolTransactions(pendingBlock.transactions, auction);

    // Claim the NFT
    await auction.connect(player).claimNFT();
  });
