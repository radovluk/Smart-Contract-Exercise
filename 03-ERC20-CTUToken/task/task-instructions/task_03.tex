...
\subsection*{Errors}
\begin{itemize}
  \item \texttt{TransferToZeroAddress}: Attempting to transfer to the zero address.
  \item \texttt{TransferFromZeroAddress}: Attempting to transfer from the zero address.
  \item \texttt{InsufficientBalance(uint256 requested, uint256 available)}: Account does not have enough balance.
  \item \texttt{ApproveToZeroAddress}: Attempting to approve the zero address as a spender.
  \item \texttt{IncreaseAllowanceForZeroAddress}: Attempting to increase allowance for the zero address.
  \item \texttt{DecreaseAllowanceForZeroAddress}: Attempting to decrease allowance for the zero address.
  \item \texttt{DecreasedAllowanceBelowZero(uint256 requested, uint256 current)}: Attempting to decrease allowance below the current value.
  \item \texttt{TransferExceedsAllowance(uint256 requested, uint256 allowance)}: Trying to transfer an amount exceeding the current allowance.
\end{itemize}
