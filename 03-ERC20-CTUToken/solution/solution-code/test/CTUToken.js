...
describe("CTUToken Contract Test Suite", function () {
  ...
  beforeEach(async function () {
    ({ ctuToken, owner, addr1, addr2, addr3 } = await loadFixture(
      deployCTUTokenFixture
    ));
  });
  ...
  describe("ERC-20 Functions", function () {
    ...
    describe("transfer", function () {
      it("Should transfer tokens successfully and update balances", async function () {
        ...
        await expect(() =>
          ctuToken.transfer(addr1.address, transferAmount)
      ...
      it("Should allow transferring zero tokens", async function () {
        ...
        expect(await ctuToken.balanceOf(owner.address)).to.equal(
          await ctuToken.totalSupply()
        );
    ...
    describe("approve and allowance", function () {
      it("Should approve allowance correctly", async function () {
        ...
        expect(await ctuToken.allowance(owner.address, addr1.address)).to.equal(
          approveAmount
        );
      });
    ...
    describe("transferFrom", function () {
      ...
      it("Should allow multiple transfers up to the allowance", async function () {
        ...
        await expect(
          ctuToken
            .connect(addr1)
            .transferFrom(owner.address, addr2.address, firstTransfer)
        )
          .to.emit(ctuToken, "Transfer")
        ...
        await expect(
          ctuToken
            .connect(addr1)
            .transferFrom(owner.address, addr2.address, secondTransfer)
        )
          .to.emit(ctuToken, "Transfer")
          .withArgs(owner.address, addr2.address, secondTransfer);

        expect(await ctuToken.balanceOf(addr2.address)).to.equal(
  ...
  describe("Edge Cases", function () {
    it("Should handle multiple approvals correctly", async function () {
      ...
      expect(await ctuToken.allowance(owner.address, addr2.address)).to.equal(
        approveAmount2
      );
    });
