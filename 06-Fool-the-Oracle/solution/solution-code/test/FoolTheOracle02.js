...
describe("Fool the Oracle Challenge 02", function () {
  ...
  before("Set up the challenge", async function () {
    ...
    console.log(
      ...
      ethers.formatEther(
        await flashLoanProvider.maxFlashLoan(usdcToken.target)
      ),
      "USDC"
    );
    ...
    console.log(
      `Player's initial balance: ${ethers.formatEther(
        PLAYER_INITIAL_ETH
      )} ETH and ` + `${ethers.formatEther(PLAYER_INITIAL_USDC)} USDC`
    );
  });
  ...
  it("Execution of player's exploit", async function () {
    ...
    const FlashLoanNFTAttacker = await ethers.getContractFactory(
      "FlashLoanNFTAttacker",
      player
    );
    ...
    const flashLoanFee = await flashLoanProvider.flashFee(
      usdcToken.target,
      borrowAmount
    );
    ...
    console.log(
      `Estimated ETH needed: ${ethers.formatEther(estimatedEthNeeded)} ETH`
    );
    ...
    console.log(
      `Sent ${ethers.formatEther(ethToSend)} ETH to attacker contract`
    );
