...
a public function. To compile the contract, run `npx hardhat compile`
in your terminal.
...
Create a new directory named `test` in your project root and add a file
called `Greeter.js` with the following content:
...
Modify your `hardhat.config.js` file to include the Sepolia network
configuration:
