...
\begin{enumerate}
  \item Visit the following \href{https://gitlab.fel.cvut.cz/radovluk/smart-contracts-exercises/-/tree/foundry/02-Decentralized-Voting-System/task/task-code}{GitLab repository} and clone it to your local machine.
...
Your implementation will be in the file \texttt{src/Voting.sol}. In this file, there are \#TODO comments where you should implement the required functionality. To fulfill this task, you need to pass all the provided tests. You can run the tests with the following command:

\begin{minted}[bgcolor=gray!5, fontsize=\footnotesize]{bash}
  $ forge test
\end{minted}

There is also a deployment script in the \texttt{script} folder. You can deploy the contract to the local foundry network with the following command:
\begin{minted}[bgcolor=gray!5, fontsize=\footnotesize]{bash}
